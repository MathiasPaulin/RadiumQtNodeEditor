...
FlowScene::createConnection( PortType connectedPort, Node& node, PortIndex portIndex ) {
...
std::shared_ptr<Connection> FlowScene::createConnection( Node& nodeIn,
    ...
    auto connection =
        std::make_shared<Connection>( nodeIn, portIndexIn, nodeOut, portIndexOut, converter );
...
void FlowScene::importConnection( const QString& fromNodeId,
                                  int fromNodePort,
    ...
    auto connection = std::make_shared<Connection>(
        *nodeIn, toNodePort, *nodeOut, fromNodePort, TypeConverter {} );
...
Node& FlowScene::importNode( std::unique_ptr<NodeDataModel>&& dataModel ) {
    auto node = detail::make_unique<Node>( std::move( dataModel ) );
    auto ngo  = detail::make_unique<NodeGraphicsObject>( *this, *node );
    node->setGraphicsObject( std::move( ngo ) );
    node->import();
...
std::vector<Node*> FlowScene::selectedNodes() const {
    ...
    for ( QGraphicsItem* item : graphicsItems ) {
        ...
        if ( ngo != nullptr ) { ret.push_back( &ngo->node() ); }
    }
...
void FlowScene::clearScene() {
    // Manual node cleanup. Simply clearing the holding datastructures doesn't work, the code
    // crashes when
    // there are both nodes and connections in the scene. (The data propagation internal logic tries
...
void FlowScene::save() const {
    ...
    if ( !fileName.isEmpty() ) {
        ...
        QFile file( fileName );
        if ( file.open( QIODevice::WriteOnly ) ) { file.write( saveToMemory() ); }
    }
}
...
Node* locateNodeAt( QPointF scenePoint, FlowScene& scene, QTransform const& viewTransform ) {
    ...
    std::copy_if(
        items.begin(), items.end(), std::back_inserter( filteredItems ), []( QGraphicsItem* item ) {
            ...
            // Ugly workaround to prevent segfault in the following dynamic_cast
            return ( typeid( *item ).hash_code() == typeid( NodeGraphicsObject ).hash_code() );
#else
